...
def main():
    ...
    print("Running 10 Gabor parameter combinations (frequency, sigma)...")
    print("Each figure: RGB, Gabor slope-stripes, DEM, Slope, Slope orientation, 10x10 mean (2x3 grid)")
    print()
    results = []

    for i, (frequency, sigma) in enumerate(combinations, 1):
        label = f"freq{_param_to_str(frequency)}sigma{_param_to_str(sigma)}"
        ...
        results.append((frequency, sigma, mean_val, min_val, max_val, label))
        ...
        ax1.set_title(f"Gabor slope-stripes  freq={frequency}  sigma={sigma}")
        ax1.axis("off")
        ...
        fig.suptitle(f"gabor_slope_stripes_sample_{label}.png", fontsize=12)
        out_path = out_dir / f"gabor_slope_stripes_sample_{label}.png"
        fig.savefig(out_path, dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"  {i:2d}/10  freq={frequency:.2f}  sigma={sigma:.1f}  mean={mean_val:.4f}  ->  {out_path.name}")

    print()
    print("Summary (mean value in [0,1]):")
    print("  freq   sigma   mean    min    max   filename")
    for frequency, sigma, mean_val, min_val, max_val, label in results:
        print(f"  {frequency:.2f}   {sigma:5.1f}     {mean_val:.3f}   {min_val:.3f}   {max_val:.3f}   gabor_slope_stripes_sample_{label}.png")
    print()
