...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: $0 [--skip-baselines] [--max-tiles N] [--source dev|prod] [--tile-size 256|512] [--config PATH]" >&2
      exit 1
      ;;
  esac
done
...
poetry run python scripts/train_model.py --mode synthetic_parenthesis --config "$TRAINING_CONFIG" --tile-size "$TILE_SIZE"
